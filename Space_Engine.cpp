...
//
//  This file is part of the SpaceEngine project written in C++.
//  SpaceEngine is a simple OpenGL-based simulation engine for educational purposes.
//  It demonstrates basic physics and rendering of space using OpenGL and GLFW.
//
//  SpaceEngine is free software: you can redistribute it and/or modify
//  it under the terms of the GNU General Public License as published by
//  the Free Software Foundation, either version 3 of the License, or
//  (at your option) any later version.
//
//  Updated for 3D rendering with camera controls and shaders
//
//  Controls:
...
////////////////////// INSERT CODE FROM ANY ONE OF THE C++ FILES IN THIS REPOSITORY INTO HERE TO RUN IT //////////////////////
...
// Simulation control
bool isPaused = false;
float simulationSpeed = 0.1f;     // Reduced from 1.0f to slow down simulation
float G = 1.0f;                   // Reduced gravitational constant for slower orbits
const float MAX_TIMESTEP = 0.01f; // Increased timestep for smoother motion
...
std::vector<Object3D> CreateObjects()
{
    ...
    objects.emplace_back(
        ...
        glm::vec3(0.0f),
        1000.0f, // Reduced mass for slower orbits
    ...
    float r1 = 5.0f;
    float v1 = sqrt(G * sunMass / r1) * 0.7f; // Further reduced velocity
    objects.emplace_back(
    ...
    float r2 = 8.0f;
    float v2 = sqrt(G * sunMass / r2) * 0.7f; // Reduced velocity
    objects.emplace_back(
    ...
    float r3 = 12.0f;
    float v3 = sqrt(G * sunMass / r3) * 0.7f; // Reduced velocity
    objects.emplace_back(
    ...
    float r4 = 16.0f;
    float v4 = sqrt(G * sunMass / r4) * 0.65f; // Reduced elliptical velocity
    objects.emplace_back(
        glm::vec3(r4, 0.0f, 0.0f),
        glm::vec3(0.0f, 0.05f, v4), // Reduced y-component
        18.0f,
    ...
    float moonOrbitalSpeed = sqrt(G * 15.0f / moonOrbitRadius) * 0.5f; // Reduced moon speed
    objects.emplace_back(
    ...
    // Asteroid belt objects (reduced velocity)
    for (int i = 0; i < 8; i++)
    {
        float angle = i * 2.0f * M_PI / 8.0f;
        float asteroidR = 9.5f + 0.3f * (rand() / float(RAND_MAX) - 0.5f);
        float asteroidV = sqrt(G * sunMass / asteroidR) * (0.6f + 0.1f * (rand() / float(RAND_MAX))); // Reduced asteroid velocities
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        if (!isPaused)
        {
            ...
            lightAngle += deltaTime * 0.1f; // Much slower light rotation
        }
...
GLFWwindow *StartGLFW()
{
    ...
    std::cout << "C: Show simulation stats" << std::endl;
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mods)
{
    if (action == GLFW_PRESS)
    {
        ...
        else if (key == GLFW_KEY_EQUAL || key == GLFW_KEY_KP_ADD) // + key
        {
            simulationSpeed = std::min(simulationSpeed * 1.5f, 2.0f);
            std::cout << "Simulation speed increased to: " << simulationSpeed << std::endl;
        }
        else if (key == GLFW_KEY_MINUS || key == GLFW_KEY_KP_SUBTRACT) // - key
        {
            simulationSpeed = std::max(simulationSpeed / 1.5f, 0.01f);
            std::cout << "Simulation speed decreased to: " << simulationSpeed << std::endl;
        }
    }
}